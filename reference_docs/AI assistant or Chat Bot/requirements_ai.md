...
_Все минимальные требования ПЛЮС:_
...
_Минимум один дополнительный тип input/output:_
...
**⚠️ ОСНОВНЫЕ ОШИБКИ И НЕДОЧЕТЫ**

**Критические ошибки в безопасности**

- **Хранение API ключей в коде или репозитории**  
    _Пример:_ api_key = "sk-xxxxx" прямо в коде  
    **Минус 2 балла**
- **Отсутствие защиты от prompt injection**  
    _Пример:_ Прямая передача пользовательского ввода без санитизации  
    **Минус 1 балл**
- **Логирование sensitive данных**  
    _Пример:_ Полные данные пользователей в логах  
    **Минус 1 балл**

**Ошибки в архитектуре**

- **Отсутствие обработки ошибок API**  
    _Пример:_ Приложение падает при 429 (rate limit) или timeout  
    **Минус 1 балл**
- **Простая proxy без добавленной ценности**  
    _Пример:_ response = openai.complete(user_input) и всё  
    **Минус 2 балла**
- **Отсутствие промпт-инжиниринга**  
    _Пример:_ Нет системного промпта, контекста, инструкций  
    **Минус 1 балл**

**Ошибки в UX/функциональности**

- **Нет индикации состояния обработки**  
    _Пример:_ Пользователь не понимает, работает ли бот  
    **Минус 1 балл**
- **Потеря контекста в рамках сессии**  
    _Пример:_ Бот забывает о чем говорили 2 сообщения назад  
    **Минус 1 балл**
- **Отсутствие валидации входных данных**  
    _Пример:_ Принимаются сообщения любой длины, крашится на emoji  
    **Минус 1 балл**

**Ошибки в реализации**

- **Синхронная блокирующая обработка**  
    _Пример:_ Весь сервер ждет ответа от OpenAI для одного пользователя  
    **Минус 1 балл**
- **Отсутствие логирования**  
    _Пример:_ Невозможно отследить проблемы в production  
    **Минус 1 балл**
- **Хардкод конфигураций**  
    _Пример:_ URL, модели, параметры зашиты в код  
    **Минус 1 балл**

**Ошибки в документации**

- **Отсутствие документации промптов**  
    _Пример:_ Непонятно какие промпты используются и почему  
    **Минус 1 балл**
- **Нет описания ограничений системы**  
    _Пример:_ Не указано что бот НЕ умеет делать  
    **Минус 1 балл**

**Ошибки в развертывании**

- **Приложение не развернуто или недоступно**  
    _Пример:_ Работает только localhost  
    **Минус 2 балла**
