...
DANGEROUS_PATTERNS: List[Tuple[re.Pattern, str, str]] = [
    ...
    (
        re.compile(
            r"you\s+are\s+now\s+(a|an|the)?\s*(new|different|evil|unrestricted)",
            re.IGNORECASE,
        ),
    ...
    (
        re.compile(
            r"pretend\s+(you\s+are|to\s+be)\s+(a|an)?\s*(different|new|evil|unrestricted)",
            re.IGNORECASE,
        ),
        "Role switching attempt",
        "block",
    ...
    (
        re.compile(
            r"act\s+as\s+(if\s+)?(you\s+are\s+)?(a|an)?\s*(different|unrestricted|evil)",
            re.IGNORECASE,
        ),
        "Role switching attempt",
        "block",
    ...
    (re.compile(r"^\s*system\s*:", re.IGNORECASE | re.MULTILINE), "System prompt injection", "block"),
    (re.compile(r"\[INST\]|\[/INST\]|\[\[SYSTEM\]\]", re.IGNORECASE), "Instruction delimiter injection", "block"),
    (re.compile(r"<\|system\|>|<\|user\|>|<\|assistant\|>", re.IGNORECASE), "Chat template injection", "block"),
    (re.compile(r"###\s*(System|Human|Assistant)\s*:", re.IGNORECASE), "Markdown role injection", "block"),
    # Jailbreak patterns
    (re.compile(r"(DAN|Developer)\s*mode", re.IGNORECASE), "Jailbreak attempt (DAN mode)", "block"),
    (re.compile(r"jailbreak|jail\s*break", re.IGNORECASE), "Explicit jailbreak attempt", "block"),
    (
    ...
    (
        re.compile(
            r"(print|output|return|say|show|display)\s+your\s+(system\s+)?(prompt|instructions?|rules?)",
            re.IGNORECASE,
        ),
        "System prompt extraction attempt",
        "block",
    ),
    (re.compile(r"reveal\s+(your\s+)?(instructions?|system\s*prompt|rules?)", re.IGNORECASE), "Instruction extraction attempt", "block"),
    (re.compile(r"what\s+(are|is)\s+your\s+(system\s+)?prompt", re.IGNORECASE), "System prompt query", "block"),
    # Encoding tricks
    (re.compile(r"base64\s*:\s*[A-Za-z0-9+/=]{20,}", re.IGNORECASE), "Encoded payload attempt", "block"),
    (re.compile(r"\\x[0-9a-fA-F]{2}(\\x[0-9a-fA-F]{2}){5,}", re.IGNORECASE), "Hex-encoded payload", "block"),
]
